 ...
 * TODO: Javadoc
 */
class DialogExampleFragment : GuidedStepFragment() {

    override fun onCreateGuidance(savedInstanceState: Bundle?): Guidance {
        return Guidance(getString(R.string.dialog_example_title),
    ...
    override fun onGuidedActionClicked(action: GuidedAction?) {
        if (ACTION_ID_POSITIVE.toLong() == action!!.id) {
            Toast.makeText(activity, R.string.dialog_example_button_toast_positive_clicked,
                    Toast.LENGTH_SHORT).show()
        } else {
            Toast.makeText(activity, R.string.dialog_example_button_toast_negative_clicked,
                    Toast.LENGTH_SHORT).show()
        }
        activity.finish()
    }
